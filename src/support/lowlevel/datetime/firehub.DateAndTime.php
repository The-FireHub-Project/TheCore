...
final class DateAndTime {
     ...
     */
    public static function stringToTimestamp (string $datetime):int|false {
     ...
     *
     * @throws Error If format cannot be to integer.
     *
     * @return int<0, max> Formatted date as integer.
     */
    public static function formatInteger (string $format, ?int $timestamp = null):int {

        return (($ideate = idate($format, $timestamp)) !== false) ? $ideate : throw new Error("Cannot convert $format to integer.");
     ...
     * @since 0.1.3.pre-alpha.M1
     *
     * @throws Error If timezone doesn't exist.
     *
     * @return \FireHub\TheCore\Support\Enums\DateTime\TimeZone Timezone.
     */
    public static function getDefaultTimezone ():TimeZone {

        $default = date_default_timezone_get();

        return ($time_zone = TimeZone::tryFrom($default)) ? $time_zone : throw new Error("Timezone $default doesn't exist.");
